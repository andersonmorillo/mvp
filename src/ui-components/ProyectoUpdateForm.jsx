...
export default function ProyectoUpdateForm(props) {
  const {
    id: idProp,
    proyecto,
    onSuccess,
  ...
  const [estudiante, setEstudiante] = React.useState(initialValues.estudiante);
  const [profesor, setProfesor] = React.useState(initialValues.profesor);
  const [errors, setErrors] = React.useState({});
  const resetStateValues = () => {
    const cleanValues = proyectoRecord
      ? { ...initialValues, ...proyectoRecord }
      : initialValues;
    setNombre(cleanValues.nombre);
    setDescripcion(cleanValues.descripcion);
    setEstado(cleanValues.estado);
    setCampo(cleanValues.campo);
    setEstudiante(cleanValues.estudiante);
    setProfesor(cleanValues.profesor);
    setErrors({});
  };
  const [proyectoRecord, setProyectoRecord] = React.useState(proyecto);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Proyecto, idProp)
        : proyecto;
      setProyectoRecord(record);
    };
    queryData();
  }, [idProp, proyecto]);
  React.useEffect(resetStateValues, [proyectoRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Proyecto.copyOf(proyectoRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ProyectoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nombre"
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.estudiante ?? value;
          }
          if (errors.estudiante?.hasError) {
            runValidationTasks("estudiante", value);
          }
          setEstudiante(value);
        }}
        onBlur={() => runValidationTasks("estudiante", estudiante)}
        errorMessage={errors.estudiante?.errorMessage}
        hasError={errors.estudiante?.hasError}
        {...getOverrideProps(overrides, "estudiante")}
      ></TextField>
      <TextField
        label="Profesor"
        isRequired={false}
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.profesor ?? value;
          }
          if (errors.profesor?.hasError) {
            runValidationTasks("profesor", value);
          }
          setProfesor(value);
        }}
        onBlur={() => runValidationTasks("profesor", profesor)}
        errorMessage={errors.profesor?.errorMessage}
        hasError={errors.profesor?.hasError}
        {...getOverrideProps(overrides, "profesor")}
      ></TextField>
      <Flex
