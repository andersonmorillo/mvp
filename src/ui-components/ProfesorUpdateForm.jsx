...
export default function ProfesorUpdateForm(props) {
  const {
    id: idProp,
    profesor,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = profesorRecord
      ? { ...initialValues, ...profesorRecord }
      : initialValues;
  ...
  const [profesorRecord, setProfesorRecord] = React.useState(profesor);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Profesor, idProp)
        : profesor;
      setProfesorRecord(record);
    };
    queryData();
  }, [idProp, profesor]);
  React.useEffect(resetStateValues, [profesorRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Profesor.copyOf(profesorRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ProfesorUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nombre"
